...
public class DocumentGeneratorWorkerActor extends UntypedActor {
	...
	public DocumentGeneratorWorkerActor(
			final SampleDataGeneratorService sampleDataGenerator) {
		this.sampleDataGenerator = sampleDataGenerator;
	}

	@Override
	public void onReceive(final Object message) {
		// LOG.debug("Worker Actor message for DocumentGeneratorWorkerActor is:"
		// + message);
		if (message instanceof IndexDocumentVO) {
			try {
				...
				case PRODUCT:
					final Product product = sampleDataGenerator
							.generateProductSampleDataFor(indexDocumentVO
									.getDocumentId());
				...
				case PRODUCT_PROPERTY:
					final ProductProperty productProperty = sampleDataGenerator
							.generateProductPropertySampleDataFor(indexDocumentVO
									.getDocumentId());
				...
				case PRODUCT_GROUP:
					final ProductGroup productGroup = sampleDataGenerator
							.generateProductGroupSampleDataFor(indexDocumentVO
									.getDocumentId());
				...
				LOG.error(
						e,
						"Error occurred while generating document for message: {}",
						message);
				final DocumentGenerationException documentGenerationException = new DocumentGenerationException(
						e);
