...
public class SetupDocumentTypeWorkerActor extends UntypedActor {
	...
	public SetupDocumentTypeWorkerActor(
		...
		dataGeneratorWorkerRouter = getContext().actorOf(
				Props.create(DataGeneratorWorkerActor.class,
						sampleDataGeneratorService), "dataGeneratorWorker");
		documentGeneratorWorkerRouter = getContext()
				.actorOf(
						Props.create(DocumentGeneratorWorkerActor.class,
		...
		indexDocumentWorkerRouter = getContext().actorOf(
				Props.create(IndexProductDataWorkerActor.class,
						indexProductDataService).withRouter(new FromConfig())
						.withDispatcher("indexDocumentWorkerActorDispatcher"),
				"indexDocumentWorker");
	}
	...
	public void onReceive(final Object message) {
		// LOG.debug("Worker Actor message for SetupDocumentTypeWorkerActor is:"
		// + message);
		try {
			...
			final String errorMessage = "Error occured while indexing document type: "
					+ message;
			LOG.error(exception, errorMessage);
			final DocumentTypeIndexingException documentTypeIndexingException = new DocumentTypeIndexingException(
					indexDocumentType, errorMessage, exception);
	...
	private void handleExceptionInChildActors(final Object message) {
		...
		if (ex instanceof DocumentTypeDataGenerationException) {
			// issue in generating data itself for a document type. As each
			// worker only handling one document type, tell parent that it is
			// done.
			// TODO: check proper handling
			final DocumentTypeIndexingException documentTypeIndexingException = new DocumentTypeIndexingException(
					indexDocumentType,
					"Data generation failed, failing whole document type itself!",
					ex);
			sendMessageToParent(documentTypeIndexingException);
			resetActorState();
	...
	private void generateDocumentAndIndexDocument(final Object message) {
		...
		if (!indexDocumentVO.isIndexDone()) {
			...
			if (indexDocumentVO.getProduct() == null
					&& indexDocumentVO.getProductProperty() == null
	...
	private void handleDocumentTypeForDataGeneration(final Object message) {
		...
		// Check input data
		Assert.notNull(indexDocumentTypeMessageVO.getConfig(),
				"Indexing config can not be null!");
		Assert.notNull(indexDocumentTypeMessageVO.getIndexDocumentType(),
				"Document type can not be null!");
		// Check existing state.
		Assert.isNull(indexDocumentType,
				"Actor already processing document type, this should have not happened!");
		Assert.isTrue(totalDocumentsToIndex == 0,
				"Existing docs to index should have been zero!");
		Assert.isTrue(totalDocumentsToIndexDone == 0,
				"Existing docs to indexing done should have been zero!");
	...
	private void updateStateAndResetIfAllDone() {
		LOG.debug(
				"Total indexing stats for document type are: totalProductsToIndex: {}, totalProductsToIndexDone: {}",
				new Object[] { totalDocumentsToIndex, totalDocumentsToIndexDone });
		if (totalDocumentsToIndex == totalDocumentsToIndexDone) {
			LOG.debug(
					"All products indexing done for total document types {} sending message {} to parent!",
					new Object[] { indexDocumentType, indexDocumentType });
			// Find parent actor in the hierarchy.
			// akka://SearchIndexingSystem/user/setupIndexMasterActor/setupIndexWorkerActor/$a
			// Send the document type done for all the handling types, for now
			// total products done means all types done, change it.
	...
	private void sendMessageToParent(final Object message) {
		// Using actor selection to find parent , as message of indexing
		// completion is received from child actors.
		// To find parent, because of $val syntax and routee, using actor path
		// for the same.
	...
	private void handleUnhandledMessage(final Object message) {
		...
		LOG.error(
				"Unhandled message encountered in SetupDocumentTypeWorkerActor: {}",
				message);
		unhandled(message);
	}
	...
	public void setDataGeneratorWorkerRouter(
			final ActorRef dataGeneratorWorkerRouter) {
		this.dataGeneratorWorkerRouter = dataGeneratorWorkerRouter;
	}

	public void setDocumentGeneratorWorkerRouter(
			final ActorRef documentGeneratorWorkerRouter) {
		this.documentGeneratorWorkerRouter = documentGeneratorWorkerRouter;
	}

	public void setIndexDocumentWorkerRouter(
			final ActorRef indexDocumentWorkerRouter) {
