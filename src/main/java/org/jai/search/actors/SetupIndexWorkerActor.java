...
public class SetupIndexWorkerActor extends UntypedActor {
	...
	public void onReceive(final Object message) {
		LOG.debug("Worker Actor message for SetupIndexWorkerActor is:"
				+ message);
		try {
			...
			// TODO: check if it needs to be handled different, not sure how
			// much indexing for index or types is already done.
			final String errorMessage = "Error occurred while indexing: "
					+ message;
			LOG.error(ex, errorMessage);
			final IndexingException indexingException = new IndexingException(
					config, errorMessage, ex);
	...
	private void handleException(final Object message) {
		...
		if (ex instanceof DocumentTypeIndexingException) {
			documentTypesDone
					.put(((DocumentTypeIndexingException) ex)
							.getIndexDocumentType(), true);
			updateStateAndNotifyParentIfAllDone();
		} else {
			unhandled(message);
		}
	...
	private void handleEachIndex(final Object message) {
		...
		// Loop through all document types and index relevant products for
		// those.
		// eg. you want to index products/product group/specifications
		// separately.
		// This only works if no parent child stuff other order indexing
		// accordingly.
		// Index products
	...
	private void indexDocumentType(final ElasticSearchIndexConfig config,
			final IndexDocumentType indexDocumentType) {
		workerRouter.tell(new IndexDocumentTypeMessageVO().config(config)
				.documentType(indexDocumentType).newIndexName(newIndexName),
				getSelf());
	...
	private void updateStateAndNotifyParentIfAllDone() {
		...
		for (final Entry<IndexDocumentType, Boolean> entry : documentTypesDone
				.entrySet()) {
			LOG.debug(
					"Total indexing stats are: documentType: {}, DocumentTypesStatus: {}",
		...
		if (isAlldocumentTypeDone) {
			// TODO: index type should have been done. shift alising for newly
			// created indices now.
			setupIndexService.replaceAlias(newIndexName,
					config.getIndexAliasName());
			// sendMessageToParent(config);
			sendMessageToParent(config);
			LOG.debug("All indexing done for the index: {} {}", new Object[] {
					newIndexName, config });
