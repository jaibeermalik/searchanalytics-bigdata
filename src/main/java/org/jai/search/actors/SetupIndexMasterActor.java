...
public class SetupIndexMasterActor extends UntypedActor {
	...
	private void handleIndexingMessage(final Object message) {
		...
		} else if (IndexingMessage.REBUILD_ALL_INDICES_DONE
				.equals(indexingMessage)) {
	...
	private void updateIndexDoneState() {
		...
		for (final Entry<ElasticSearchIndexConfig, Boolean> entry : indexDone
				.entrySet()) {
			LOG.debug("Indexing setup current status is index: {} status: {}",
