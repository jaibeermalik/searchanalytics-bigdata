...
public class DataGeneratorWorkerActor extends UntypedActor {
	...
	public DataGeneratorWorkerActor(
	...
	public void onReceive(final Object message) {
		// LOG.debug("Worker Actor message for DataGeneratorWorkerActor is:" +
		// message);
		...
		if (message instanceof IndexDocumentTypeMessageVO) {
			...
			try {
				...
				case PRODUCT:
					generateData(indexDocumentTypeMessageVO,
							sampleDataGeneratorService
									.generateProductsSampleData().size());
					break;
				case PRODUCT_PROPERTY:
					generateData(indexDocumentTypeMessageVO,
							sampleDataGeneratorService
									.generateProductPropertySampleData().size());
					break;
				case PRODUCT_GROUP:
					generateData(indexDocumentTypeMessageVO,
							sampleDataGeneratorService
									.generateProductGroupSampleData().size());
					break;
				default:
					handleUnhandledMessage(message);
				}
			} catch (final Exception ex) {
				final String errorMessage = "Error occurred while generating data for message"
						+ message;
	...
	private void handleUnhandledMessage(final Object message) {
		...
		LOG.error(
				"Unhandled message encountered in DataGeneratorWorkerActor: {}",
				message);
		unhandled(message);
	}

	private void generateData(
			final IndexDocumentTypeMessageVO indexDocumentTypeMessageVO,
			final int size) {
		// Before starting data generation, send how much data will be
		// generated, which is size.
		getSender().tell(Integer.valueOf(size), getSelf());
		// LOG.debug("Generating data for IndexDocumentTypeMessageVO: {}, for size: {}",
		// new Object[]{indexDocumentTypeMessageVO, size});
		for (int i = 1; i <= size; i++) {
			final IndexDocumentVO indexDocumentVO = new IndexDocumentVO()
					.config(indexDocumentTypeMessageVO.getConfig())
					.documentType(
							indexDocumentTypeMessageVO.getIndexDocumentType())
					.newIndexName(indexDocumentTypeMessageVO.getNewIndexName())
					.documentId(Long.valueOf(i));
	...
	public void preRestart(final Throwable reason, final Option<Object> message)
		...
		LOG.debug("DataGeneratorWorkerActor restarted because of reason: {}",
				reason);
	}
}
