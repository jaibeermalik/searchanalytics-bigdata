...
public class IndexProductDataWorkerActor extends UntypedActor {
	...
	public IndexProductDataWorkerActor(
	...
	public void onReceive(final Object message) {
		// LOG.debug("Worker Actor message for IndexProductDataWorkerActor is:"
		// + message);
		if (message instanceof IndexDocumentVO) {
			try {
				...
				case PRODUCT:
					indexProductDataService.indexProduct(
							indexDocumentVO.getConfig(),
							indexDocumentVO.getNewIndexName(),
							indexDocumentVO.getProduct());
					indexDocumentVO.indexDone(true);
					getSender().tell(indexDocumentVO, getSelf());
					break;
				case PRODUCT_PROPERTY:
					indexProductDataService.indexProductPropterty(
							indexDocumentVO.getConfig(),
							indexDocumentVO.getNewIndexName(),
					...
					break;
				case PRODUCT_GROUP:
					indexProductDataService.indexProductGroup(
							indexDocumentVO.getConfig(),
							indexDocumentVO.getNewIndexName(),
				...
				LOG.error(
						e,
						"Error occured while indexing document data for message: {}",
						message);
				final IndexDataException indexDataException = new IndexDataException(
						e);
