...
@Service
public class SampleDataGeneratorServiceImpl implements
		SampleDataGeneratorService {
	private static final Logger logger = LoggerFactory
			.getLogger(SampleDataGeneratorServiceImpl.class);
	...
	private static List<Product> generateSampleData() {
		...
		for (int i = 1; i <= 50; i++) {
			...
			if (i < 5) {
				product.addCategory(findCategory(hierarchicalCategories,
						MACBOOK_AIR));
				product.addCategory(findCategory(hierarchicalCategories, APPLE));
				product.addCategory(findCategory(hierarchicalCategories, RED));
				product.addCategory(findCategory(hierarchicalCategories,
						AGE_18_PLUS_YEARS));
				product.addProductProperty(findProductProperty(
						productProperties, PRODUCTPROPERTY_SIZE_21_INCH,
						PRODUCTPROPERTY_COLOR_BROWN));
				product.addProductProperty(findProductProperty(
						productProperties, PRODUCTPROPERTY_SIZE_17_INCH,
						PRODUCTPROPERTY_COLOR_PURPLE));
				product.addSpecification(new Specification(RESOLUTON_3200_1800,
						MEMORY_8_GB));
				product.addSpecification(new Specification(RESOLUTON_1920_1200,
						MEMORY_6_GB));
			} else if (i >= 5 && i <= 10) {
				product.addCategory(findCategory(hierarchicalCategories,
						MACBOOK_PRO));
				...
				product.addProductProperty(findProductProperty(
						productProperties, PRODUCTPROPERTY_SIZE_15_INCH,
						PRODUCTPROPERTY_COLOR_YELLOW));
				product.addProductProperty(findProductProperty(
						productProperties, PRODUCTPROPERTY_SIZE_17_INCH,
						PRODUCTPROPERTY_COLOR_PURPLE));
				product.addSpecification(new Specification(RESOLUTON_1920_1080,
						MEMORY_6_GB));
				product.addSpecification(new Specification(RESOLUTON_1920_1200,
						MEMORY_6_GB));
			} else if (i > 10 && i <= 20) {
				...
				product.addCategory(findCategory(hierarchicalCategories,
						AGE_12_18_YEARS));
				product.addProductProperty(findProductProperty(
						productProperties, PRODUCTPROPERTY_SIZE_12_INCH,
						PRODUCTPROPERTY_COLOR_BLACK));
				product.addSpecification(new Specification(RESOLUTON_1920_1080,
						MEMORY_4_GB));
				product.addSpecification(new Specification(RESOLUTON_1920_1080,
						MEMORY_2_GB));
				...
				product.addCategory(findCategory(hierarchicalCategories,
						AGE_0_12_YEARS));
				product.addProductProperty(findProductProperty(
						productProperties, PRODUCTPROPERTY_SIZE_13_INCH,
						PRODUCTPROPERTY_COLOR_GREY));
				product.addSpecification(new Specification(RESOLUTON_1024_758,
						MEMORY_2_GB));
				product.addSpecification(new Specification(RESOLUTON_1024_600,
						MEMORY_6144_MB));
			}
	...
	private static ProductProperty findProductProperty(
			final Set<ProductProperty> productProperties, final String size,
			final String color) {
		for (final ProductProperty productProperty : productProperties) {
			if (size.equals(productProperty.getSize())
					&& color.equals(productProperty.getColor())) {
	...
	@Override
	public ProductProperty findProductProperty(final String size,
			final String color) {
	...
	@Override
	public ProductProperty generateProductPropertySampleDataFor(
			final Long productPropertyId) {
		for (final ProductProperty productProperty : productProperties) {
	...
	@Override
	public ProductGroup generateProductGroupSampleDataFor(
	...
	private static Set<Category> generateHierarchicalCategoryData() {
		final Category computerCategory = new Category(COMPUTER, null,
				SearchFacetName.SEARCH_FACET_TYPE_PRODUCT_TYPE.getCode());
		final Category laptops = new Category(LAPTOPS, computerCategory,
				SearchFacetName.SEARCH_FACET_TYPE_PRODUCT_TYPE.getCode());
		final Category macbookLaptops = new Category(MACBOOK, laptops,
				SearchFacetName.SEARCH_FACET_TYPE_PRODUCT_TYPE.getCode());
		final Category macbookProLaptops = new Category(MACBOOK_PRO,
				macbookLaptops,
				SearchFacetName.SEARCH_FACET_TYPE_PRODUCT_TYPE.getCode());
		final Category macbookAirLaptops = new Category(MACBOOK_AIR,
				macbookLaptops,
				SearchFacetName.SEARCH_FACET_TYPE_PRODUCT_TYPE.getCode());
		final Category chrmoebookLaptops = new Category(CHROMEBOOK, laptops,
				SearchFacetName.SEARCH_FACET_TYPE_PRODUCT_TYPE.getCode());
		final Category netbookLaptops = new Category(NETBOOK, laptops,
				SearchFacetName.SEARCH_FACET_TYPE_PRODUCT_TYPE.getCode());
		final Category brands = new Category(BRANDS, null,
				SearchFacetName.SEARCH_FACET_TYPE_BRAND.getCode());
		final Category appleBrand = new Category(APPLE, brands,
				SearchFacetName.SEARCH_FACET_TYPE_BRAND.getCode());
		final Category hpBrand = new Category(HP, brands,
				SearchFacetName.SEARCH_FACET_TYPE_BRAND.getCode());
		final Category dellBrand = new Category(DELL, brands,
				SearchFacetName.SEARCH_FACET_TYPE_BRAND.getCode());
		final Category age = new Category(AGE, null,
				SearchFacetName.SEARCH_FACET_TYPE_AGE.getCode());
		final Category kidAge = new Category(AGE_0_12_YEARS, age,
				SearchFacetName.SEARCH_FACET_TYPE_AGE.getCode());
		final Category teenAge = new Category(AGE_12_18_YEARS, age,
				SearchFacetName.SEARCH_FACET_TYPE_AGE.getCode());
		final Category adultAge = new Category(AGE_18_PLUS_YEARS, age,
				SearchFacetName.SEARCH_FACET_TYPE_AGE.getCode());
		final Category colors = new Category(COLORS, null,
				SearchFacetName.SEARCH_FACET_TYPE_COLOR.getCode());
		final Category redColor = new Category(RED, colors,
				SearchFacetName.SEARCH_FACET_TYPE_COLOR.getCode());
		final Category greenColor = new Category(GREEN, colors,
				SearchFacetName.SEARCH_FACET_TYPE_COLOR.getCode());
		final Category blueColor = new Category(BLUE, colors,
				SearchFacetName.SEARCH_FACET_TYPE_COLOR.getCode());
	...
	private static Category findCategory(final Set<Category> categories,
