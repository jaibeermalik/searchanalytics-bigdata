...
public class SearchClientServiceMockImpl implements SearchClientService {
	...
	private Settings defaultSettings = ImmutableSettings
			...
			.put(ElasticSearchReservedWords.PATH_DATA.getText(),
					new File(System.getProperty("java.io.tmpdir")
							+ "/esintegrationtest/data").getAbsolutePath())
			.put(ElasticSearchReservedWords.PATH_WORK.getText(),
					new File(System.getProperty("java.io.tmpdir")
							+ "/esintegrationtest/work").getAbsolutePath())
			.put(ElasticSearchReservedWords.PATH_LOG.getText(),
					new File(System.getProperty("java.io.tmpdir")
							+ "/esintegrationtest/log").getAbsolutePath())
			.put(ElasticSearchReservedWords.PATH_CONF.getText(),
					new File("config").getAbsolutePath())
			// will not survive restart
			// TODO: memory store type cause out of memory in eclipse on low
			// config machine
			// Check how to set memory setting and allocations in memory store
			// type.
	...
	public void setup() {
		try {
			...
			throw new RuntimeException(
					"Error occured while starting ES server!", e);
		}
	}
	...
	private void createNodes() throws Exception {
		...
		final Settings settings = settingsBuilder()
				...
				// .put(ElasticSearchReservedWords.INDEX_MAPPER_DYNAMIC.getText(),
				// false)
				.put("http.enabled", true)
				.put("cluster.name", "jai-testclusterName")
				.put("node.name", "jai-eslocalnode")
				.put("transport.tcp.port", 9310).put("http.port", 9210)
				.put("network.host", "localhost").build();
