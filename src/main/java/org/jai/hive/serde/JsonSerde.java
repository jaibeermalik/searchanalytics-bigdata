...
/**
 * This SerDe can be used for processing JSON data in Hive. It supports arbitrary JSON data, and can handle all Hive types except for UNION.
 * However, the JSON data is expected to be a series of discrete records, rather than a JSON array of objects.
 *
 * The Hive table is expected to contain columns with names corresponding to fields in the JSON data, but it is not necessary for every JSON
 * field to have a corresponding Hive column. Those JSON fields will be ignored during queries.
 *
 * Example:
 *
 ...
 *
 * JSON objects can also interpreted as a Hive MAP type, so long as the keys and values in the JSON object are all of the appropriate types.
 * For example, in the JSON above, another valid table declaraction would be:
 *
 * CREATE TABLE foo (a INT, b ARRAY<STRING>, c MAP<STRING,STRING>);
 *
 * Only STRING keys are supported for Hive MAPs. TODO: add case insensitive for using this in hive.
 */
public class JsonSerde extends AbstractSerDe
{
    ...
    /**
     * An initialization function used to gather information about the table. Typically, a SerDe implementation will be interested in the
     * list of column names and their types. That information will be used to help perform actual serialization and deserialization of data.
     */
    @Override
    public void initialize(final Configuration conf, final Properties tbl) throws SerDeException
    {
        ...
        final String colTypesStr = tbl.getProperty(serdeConstants.LIST_COLUMN_TYPES);
        final List<TypeInfo> colTypes = TypeInfoUtils.getTypeInfosFromTypeString(colTypesStr);
        rowTypeInfo = (StructTypeInfo) TypeInfoFactory.getStructTypeInfo(colNames, colTypes);
        rowOI = TypeInfoUtils.getStandardJavaObjectInspectorFromTypeInfo(rowTypeInfo);
    }

    /**
     * This method does the work of deserializing a record into Java objects that Hive can work with via the ObjectInspector interface. For
     * this SerDe, the blob that is passed in is a JSON string, and the Jackson JSON parser is being used to translate the string into Java
     * objects.
     *
     * The JSON deserialization works by taking the column names in the Hive table, and looking up those fields in the parsed JSON object.
     * If the value of the field is not a primitive, the object is parsed further.
     */
    @Override
    public Object deserialize(final Writable blob) throws SerDeException
    {
        ...
        for (final String fieldName : rowTypeInfo.getAllStructFieldNames())
        {
            try
            {
                final TypeInfo fieldTypeInfo = rowTypeInfo.getStructFieldTypeInfo(fieldName);
    ...
    private Object parseField(Object field, final TypeInfo fieldTypeInfo)
    {
        switch (fieldTypeInfo.getCategory())
        {
            case PRIMITIVE:
                ...
                if (fieldTypeInfo.getTypeName().equalsIgnoreCase(Constants.BIGINT_TYPE_NAME))
                {
     ...
     */
    private Object parseStruct(final Object field, final StructTypeInfo fieldTypeInfo)
    {
        final Map<Object, Object> map = (Map<Object, Object>) field;
        final ArrayList<TypeInfo> structTypes = fieldTypeInfo.getAllStructFieldTypeInfos();
        final ArrayList<String> structNames = fieldTypeInfo.getAllStructFieldNames();
    ...
    /**
     * Parse a JSON list and its elements. This uses the Hive metadata for the list elements to determine how to parse the elements.
     *
     * @param field
     ...
     */
    private Object parseList(final Object field, final ListTypeInfo fieldTypeInfo)
    {
    ...
    /**
     * Parse a JSON object as a map. This uses the Hive metadata for the map values to determine how to parse the values. The map is assumed
     * to have a string for a key.
     *
     * @param field
     *            - The JSON list to parse
     * @param fieldTypeInfo
    ...
    /**
     * This method takes an object representing a row of data from Hive, and uses the ObjectInspector to get the data for each column and
     * serialize it. This implementation deparses the row into an object that Jackson can easily serialize into a JSON blob.
     */
    @Override
    ...
    /**
     * Deparse a Hive object into a Jackson-serializable object. This uses the ObjectInspector to extract the column data.
     *
     * @param obj
    ...
    /**
     * Deparses a row of data. We have to treat this one differently from other structs, because the field names for the root object do not
     * match the column names for the Hive table.
     *
     * @param obj
     ...
     */
    private Object deparseStruct(final Object obj, final StructObjectInspector structOI, final boolean isRow)
    {
        ...
        for (int i = 0; i < fields.size(); i++)
        {
            ...
            final String fieldName = isRow ? colNames.get(i) : field.getFieldName();
     ...
     * @return - A deparsed object
     */
    private Object deparsePrimitive(final Object obj, final PrimitiveObjectInspector primOI)
    {
     ...
     * @return - A deparsed object
     */
    private Object deparseList(final Object obj, final ListObjectInspector listOI)
    {
